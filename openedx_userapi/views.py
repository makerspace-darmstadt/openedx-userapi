...
from shoppingcart.models import RegistrationCodeRedemption, CourseRegistrationCode
    ...
    def post(self, request):
        ...
        return Response(
            data={
                ...
                "course_url": reverse(
                    "about_course", kwargs={"course_id": request.data.get("course_id")}
                ),
            }
        )
    ...
    def post(self, request):
        ...
        try:
            (
                ...
                course_registration,
            ) = get_reg_code_validity(enrollment_code, request, limiter)
        except Http404:
...
class EnrollmentCodeStatusView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                data={
                    "reason": "The enrollment code ({code}) was not found".format(
                        code=code
                    ),
                    "success": False,
                },
...
class GetBatchEnrollmentDataView(APIView):
    ...
    def get(self, request):
        ...
        user_ids_with_certs = GeneratedCertificate.objects.filter(
            **cert_query_filter
        ).values("user_id")
